...
## 6A. withOhm intellectual property

withOhm / Ohm retains all right, title, and interest in the Service, software, documentation, APIs, cache-tree mechanisms, metering and receipt systems, compliance gates, brand, and related intellectual property — including copyrights, trademarks, trade secrets, and any patents or applications. The MIT License covering public repository source grants copyright permissions for that source only; it does not transfer ownership of the Service, grant free hosted access, or (unless a separate writing says so) license patents or trademarks.
