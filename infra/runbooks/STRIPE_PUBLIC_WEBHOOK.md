...
1. Stripe Dashboard → Developers → Webhooks → Add endpoint  
   ...
   Events: `checkout.session.completed`, `customer.subscription.updated`, `customer.subscription.deleted`, `invoice.payment_failed` (see [docs/STRIPE.md](../../docs/STRIPE.md)).
...
NLB-only testing is insufficient for Stripe Dashboard endpoints — Stripe requires a public HTTPS hostname (after Phase 1 DNS).
